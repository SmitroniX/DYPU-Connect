        ...
        <DashboardLayout>
            <div className="h-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto px-4 pt-4 pb-2 space-y-3">
                    {confessions.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            ...
                            <h3 className="text-xl font-bold text-[var(--ui-text)]">No confessions yet</h3>
                            <p className="text-sm text-[var(--ui-text-muted)] mt-1">Be the first to share a secret! 🤫</p>
                        </div>
                    ) : (
                        confessions.map((confession) => (
                            <div key={confession.id} className="message-row group py-2">
                                <div className="flex gap-4">
                                    {/* Anonymous avatar */}
                                    <div className="w-10 h-10 shrink-0 rounded-full bg-[var(--ui-accent-dim)] flex items-center justify-center text-[var(--ui-accent)] font-bold text-sm">
                                        {confession.anonymousName.charAt(0)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        {/* Header */}
                                        <div className="flex items-baseline gap-2 mb-1">
                                            <span className="font-medium text-[var(--ui-accent)] text-[15px]">
                                        ...
                                        {/* Embed card */}
                                        <div className="callout max-w-lg">
                                            <p className="text-[15px] text-[var(--ui-text)] leading-relaxed whitespace-pre-wrap break-words">
                                                {confession.text}
                                            </p>
                                        </div>
                                        ...
                                        <div className="mt-2 flex items-center gap-1.5">
                                            <button
                                                ...
                                                className="badge hover:border-[var(--ui-accent)] cursor-pointer"
                                            >
                ...
                <div className="px-4 pb-4 shrink-0">
                    <form className="bg-[var(--ui-bg-input)] rounded-lg" onSubmit={handleSubmit}>
                        <textarea
                            className="input bg-transparent resize-none min-h-[44px] max-h-32"
                            rows={1}
                        ...
                        <div className="flex items-center justify-between px-3 pb-2">
                            <span className="text-[11px] text-[var(--ui-text-muted)] flex items-center gap-1.5">
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[var(--ui-accent)] hover:bg-[var(--ui-accent-hover)] text-[var(--ui-bg-elevated)] text-sm font-semibold rounded transition-colors disabled:opacity-30"
                            >
