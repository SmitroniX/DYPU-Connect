        ...
        <div className="max-w-6xl mx-auto pb-12 font-sans animate-[fade-in-up_0.4s_ease-out]">
            ...
            <div className="mb-6 dc-card border-[var(--dc-accent)]/20 p-5 flex items-start gap-4">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--dc-accent-dim)] ring-1 ring-[var(--dc-accent)]/20 shrink-0">
                    <Eye className="h-5 w-5 text-[var(--dc-accent)]" />
                </div>
                <div className="flex-1">
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl font-bold tracking-tight text-[var(--dc-text-primary)]">Confession Tracker</h1>
                        {!loading && (
                    ...
                    <p className="text-sm text-[var(--dc-accent)]/70 mt-1 flex items-center gap-1.5">
            ...
            <div className="dc-card overflow-hidden">
                {loading ? (
                    <div className="p-12 flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm text-[var(--dc-text-muted)]">Loading tracking logs...</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-[var(--dc-divider)]">
                            <thead className="bg-[var(--dc-bg-tertiary)]">
                                <tr>
                                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-[var(--dc-text-muted)] uppercase tracking-wider">Real Identity</th>
                                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-[var(--dc-text-muted)] uppercase tracking-wider">Anonymous Alias</th>
                                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-[var(--dc-text-muted)] uppercase tracking-wider">Confession Content</th>
                                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-[var(--dc-text-muted)] uppercase tracking-wider">Time</th>
                                </tr>
                            </thead>
                                ...
                                {filteredLogs.map((log) => (
                                    <tr key={log.id} className="hover:bg-[var(--dc-bg-hover)] transition-colors">
                                        <td className="px-5 py-3.5 whitespace-nowrap">
                                            ...
                                            <p className="text-xs text-[var(--dc-text-muted)]">{log.email}</p>
                                        </td>
                                        <td className="px-5 py-3.5 whitespace-nowrap">
                                            <span className="inline-flex items-center rounded-lg bg-[var(--dc-bg-tertiary)] px-2.5 py-1 text-sm text-[var(--dc-text-secondary)] font-mono ring-1 ring-[var(--dc-border)]">
                                                {log.anonymousName}
                                            </span>
                                        </td>
                                        <td className="px-5 py-3.5 text-sm text-[var(--dc-text-muted)] max-w-md truncate" title={log.text}>
                                            {log.text}
                                        </td>
                                        <td className="px-5 py-3.5 whitespace-nowrap">
                                ...
                                {filteredLogs.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="px-6 py-12 text-center">
                                            <Eye className="h-10 w-10 text-[var(--dc-text-muted)] mx-auto mb-3" />
                                            <p className="text-sm text-[var(--dc-text-muted)]">No confession logs found</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
