            ...
            <div className="h-full flex flex-col">
                <ChannelHeader name="messages" description="Your private campus conversations" type="dm">
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[var(--dc-accent)] hover:bg-[var(--dc-accent-hover)] text-[var(--dc-bg-tertiary)] text-sm font-semibold rounded transition-colors"
                    >
                ...
                <div className="flex-1 overflow-y-auto">
                    {showNewChat && (
                        <div className="dc-card m-4 p-5 animate-[fade-in-up_0.3s_ease-out]">
                            <h3 className="font-bold text-[var(--dc-text-primary)] mb-4">Start a conversation</h3>
                            <div className="relative mb-4">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--dc-text-muted)]" />
                                <input
                                    type="text"
                            ...
                            <div className="max-h-60 overflow-y-auto space-y-0.5">
                                {filteredUsers.map((u) => (
                                    <button
                                        key={u.id}
                                        onClick={() => startChat(u)}
                                        className="w-full flex items-center gap-3 p-2.5 hover:bg-[var(--dc-bg-hover)] rounded text-left transition-colors"
                                    >
                                        <img
                                        ...
                                        />
                                        <div>
                                            <h4 className="text-sm font-medium text-[var(--dc-text-primary)]">{u.name}</h4>
                                            <p className="text-xs text-[var(--dc-text-muted)]">{u.field} - {u.year} - {u.division}</p>
                                        </div>
                                    </button>
                                ))}
                                {filteredUsers.length === 0 && (
                                    <p className="text-sm text-[var(--dc-text-muted)] text-center py-4">No students found.</p>
                                )}
                            </div>
                        </div>
                    )}

                    <div className="divide-y divide-[var(--dc-divider)]">
                        {chats.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-[calc(100vh-12rem)] text-center">
                                ...
                                <h3 className="text-xl font-bold text-[var(--dc-text-primary)]">No messages yet</h3>
                                <p className="text-sm text-[var(--dc-text-muted)] mt-1">Get started by creating a new chat.</p>
                            </div>
                        ) : (
                            chats.map((chat) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        className="flex items-center gap-4 px-4 py-3 hover:bg-[var(--dc-bg-hover)] transition-colors group cursor-pointer"
                                    >
                                        <div className="relative shrink-0">
                                            <img src={otherImage} alt={otherName} className="w-10 h-10 rounded-full object-cover object-center" />
                                            <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-[var(--dc-offline)] rounded-full ring-[3px] ring-[var(--dc-bg-primary)]" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline mb-0.5">
                                                <h3 className="text-[15px] font-medium text-[var(--dc-text-primary)] truncate pr-4">{otherName}</h3>
                                                <span className="text-[10px] text-[var(--dc-text-muted)] shrink-0">
                                                    {chat.updatedAt?.toDate ? chat.updatedAt.toDate().toLocaleDateString() : ''}
                                                </span>
                                            </div>
                                            <p className="text-sm text-[var(--dc-text-secondary)] truncate">{chat.lastMessage || 'Say hello! 👋'}</p>
                                        </div>
                                    </Link>
                                );
                            })
